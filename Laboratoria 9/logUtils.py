...
SessionTimeTuple = namedtuple("SessionTimeTuple", "sessionStart sessionEnd")
...
def getSimpleDate(line:str):
    splitedArguments = re.split(" +",line)
...
def getIpv4sFromLog(logNamedtuple):
        ...
        if addresses:
             octets = list(map(lambda octet: int(octet),addresses[0].split(".")))
...
def logNumberOfBytes(loggerFun):
    ...
    for log in userLogs:
        ...
        if logType==MESSAGE_TYPE.SESSION_OPENED:
            ...
            if lastClosedSession and lastOpenSession:
                sessions.append(SessionTimeTuple(lastOpenSession, lastClosedSession))
